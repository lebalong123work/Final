...
router.post("/:userId/toggle", auth, async (req, res) => {
  try {
    ...
    if (followedId === followerId) return res.status(400).json({ message: "Không thể follow chính mình" });
    ...
    res.json({ success: true, data: { following, followers: cnt.rows[0]?.followers || 0 } });
  } catch (e) {
...
router.get("/:userId/status", auth, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: { following: existed.rowCount > 0, followers: cnt.rows[0]?.followers || 0 },
    });
  } catch (e) {
