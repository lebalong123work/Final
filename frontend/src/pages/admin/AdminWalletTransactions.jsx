...
export default function AdminFinance() {
  ...
  const fetchOverview = async () => {
    if (!token) {
      setOverviewErr("Bạn cần đăng nhập admin (token không tồn tại).");
      return;
    }
    try {
      setOverviewErr("");
      ...
      if (!res.ok) throw new Error(data?.message || "Lỗi tải tổng quan");

      setOverview(data?.data || {});
    } catch (e) {
      console.error(e);
      setOverviewErr(e.message || "Không kết nối được server");
    } finally {
  ...
  const fetchTopups = async (p = 1) => {
    if (!token) {
      setTopupErr("Bạn cần đăng nhập admin (token không tồn tại).");
      return;
    }
    try {
      setTopupErr("");
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data?.message || "Lỗi tải lịch sử nạp");
      ...
      setTopupTotal(0);
      setTopupErr(e.message || "Không kết nối được server");
    } finally {
  ...
  const fetchTransactions = async (p = 1) => {
    if (!token) {
      setTxErr("Bạn cần đăng nhập admin (token không tồn tại).");
      return;
    }
    ...
    try {
      ...
      setTxTotalPages(1);
      setTxTotal(0);
      setTxErr(e.message || "Không kết nối được server");
    } finally {
      setTxLoading(false);
    }
  };
        ...
        <div className="ad-page">
          <div className="container-fluid px-4 py-4">
            ...
            <div className="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
              <div>
                <h2 className="m-0 ad-title">Quản lý Tài chính</h2>
                <div className="text-secondary small">
                  Theo dõi nạp tiền & giao dịch ví của người dùng
                </div>
              </div>
              ...
              <>
                ...
                <div className="row g-3">
                  <div className="col-12 col-md-6 col-lg-3">
                    <div className="card border-0 shadow-sm rounded-4">
                      <div className="card-body">
                        <div className="text-secondary small">Tổng nạp (success)</div>
                        <div className="fw-bold fs-4 mt-1">
                          {overviewLoading ? "…" : fmtVND(overview.totalTopup || 0)}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="col-12 col-md-6 col-lg-3">
                    <div className="card border-0 shadow-sm rounded-4">
                      <div className="card-body">
                        <div className="text-secondary small">Tổng chi mua truyện</div>
                        <div className="fw-bold fs-4 mt-1">
                          {overviewLoading ? "…" : fmtVND(overview.totalSpend || 0)}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="col-12 col-md-6 col-lg-3">
                    <div className="card border-0 shadow-sm rounded-4">
                      <div className="card-body">
                        <div className="text-secondary small">Giao dịch hôm nay</div>
                        <div className="fw-bold fs-4 mt-1">
                          {overviewLoading ? "…" : (overview.todayTransactions || 0)}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="col-12 col-md-6 col-lg-3">
                    <div className="card border-0 shadow-sm rounded-4">
                      <div className="card-body">
                        <div className="text-secondary small">Giao dịch tháng này</div>
                        <div className="fw-bold fs-4 mt-1">
                          {overviewLoading ? "…" : (overview.monthTransactions || 0)}
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="d-flex justify-content-end mt-3">
                  ...
                  >
                    <i className={`bi ${overviewLoading ? "bi-arrow-repeat" : "bi-arrow-clockwise"}`} />
                  ...
                  <div className="card-body">
                    <div className="row g-2 align-items-end">
                      <div className="col-12 col-lg-4">
                        <label className="form-label small text-secondary mb-1">Tìm kiếm</label>
                      ...
                      <div className="col-12 col-md-4 col-lg-2">
                        <label className="form-label small text-secondary mb-1">Trạng thái</label>
                 <select
...
>
  ...
  <option value="failed">failed</option>
                      ...
                      <div className="col-12 col-md-4 col-lg-2">
                        <label className="form-label small text-secondary mb-1">Từ ngày</label>
                        <input
                          type="date"
                          className="form-control"
                          value={fromTopup}
                      ...
                      <div className="col-12 col-md-4 col-lg-2">
                        <label className="form-label small text-secondary mb-1">Đến ngày</label>
                        <input
                          type="date"
                          className="form-control"
                          value={toTopup}
                      ...
                      <div className="col-12 col-lg-2 d-flex gap-2">
                        ...
                        <button
                          ...
                          title="Làm mới"
                        >
                          <i className={`bi ${topupLoading ? "bi-arrow-repeat" : "bi-arrow-clockwise"}`} />
                        </button>
                      </div>
                    </div>

                    <div className="small text-secondary mt-2">
                      Tổng: <b>{topupTotal}</b> giao dịch nạp
                    </div>
                  </div>
                </div>
                ...
                <div className="card border-0 shadow-sm rounded-4">
                  <div className="card-body">
                          ...
                          {topupLoading ? (
                            ...
                            topupRows.map((t) => {
                              ...
                              return (
                                <tr key={t.id}>
                                  <td>
                                    <div className="fw-semibold">{t.username || "—"}</div>
                                    <div className="small text-secondary">{t.email || "—"}</div>
                                  </td>
                                  ...
                                  <td className="text-end fw-bold pw-pos">+{fmtVND(Math.abs(amount))}</td>
                    ...
                    <div className="d-flex flex-wrap gap-2 justify-content-between align-items-center mt-3">
                      <div className="small text-secondary">
                        Trang {topupPage}/{topupTotalPages}
                      </div>
              ...
              <>
                ...
                <div className="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3 mt-1">
                  <div className="d-flex gap-2 align-items-center">
                    ...
                    <button
                      ...
                      disabled={txLoading}
                    >
                      <i className={`bi ${txLoading ? "bi-arrow-repeat" : "bi-arrow-clockwise"}`} />
                      Làm mới
                    </button>
                  </div>

                  <div className="small text-secondary">
                    Tổng giao dịch: <b>{txTotal}</b>
                  </div>
                </div>
                ...
                <div className="card border-0 shadow-sm rounded-4">
                  <div className="card-body">
                    ...
                    <div className="d-flex flex-wrap gap-2 justify-content-between align-items-center mt-3">
                      <div className="small text-secondary">
                        Hiển thị {txFiltered.length}/{txRows.length} dòng trên trang này
                      </div>
